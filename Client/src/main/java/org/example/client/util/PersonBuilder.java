  ...
  public static Person buildFromScript(List<String> data) throws IllegalArgumentException {
    if (data.size() < 7) { // A minimum of 7 required fields
      throw new IllegalArgumentException(
          "Insufficient data to build a Person from script. Expected at least 7 fields.");
    }
    ...
    try {
      ...
      if (data.size() >= 10
          && !(data.get(7).isBlank() && data.get(8).isBlank() && data.get(9).isBlank())) {
      ...
      if (validationError != null) {
        throw new IllegalArgumentException(
            "Validation failed for person from script: " + validationError);
      }
      ...
      throw new IllegalArgumentException(
          "Error parsing person data from script: " + e.getMessage());
    }
  }
}
