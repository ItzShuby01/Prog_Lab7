...
public class PersonIOService {
  ...
  public Person readPerson() {
    ...
    LocalDateTime creationDate =
        LocalDateTime.now(); // Creation date is client-side for consistency, server might override
    ...
    return new Person(
        null, name, coordinates, creationDate, height, eyeColor, hairColor, nationality, location);
  }
  ...
  private Coordinates readCoordinates() {
    ...
    while (true) {
      try {
        ...
        ioService.print(
            "Invalid number format. Please enter a valid integer for x and a valid double for y.");
      ...
      try {
        ...
        if (Double.isNaN(height) || Double.isInfinite(height) || height <= 0) {
          throw new IllegalArgumentException(
              "Height must be positive and finite (no NaN/Infinity).");
        }
        return height;
      } catch (NumberFormatException e) {
  ...
  private EyeColor readEyeColor() {
    while (true) {
      try {
        String input =
            ioService.readLine("Enter person eye color (RED, BLACK, ORANGE): ").toUpperCase();
        return EyeColor.valueOf(input);
      } catch (IllegalArgumentException e) {
        ioService.print(
            "Invalid input. Valid eye colors are " + Arrays.toString(EyeColor.values()) + ".");
      }
    }
  }
  ...
  private HairColor readHairColor() {
    while (true) {
      try {
        String input =
            ioService
                .readLine("Enter person hair color (GREEN, BLUE, YELLOW, BROWN): ")
                .toUpperCase();
        return HairColor.valueOf(input);
      } catch (IllegalArgumentException e) {
        ioService.print(
            "Invalid input. Valid hair colors are " + Arrays.toString(HairColor.values()) + ".");
      }
    }
  }
    ...
    while (true) {
      try {
        String input =
            ioService
                .readLine(
                    "Enter nationality (RUSSIA, GERMANY, ITALY, THAILAND, JAPAN) (press Enter to skip): ")
                .trim();
        if (input.isBlank()) {
        ...
        ioService.print(
            "Invalid input. Valid options are " + Arrays.toString(Country.values()) + ".");
      }
    }
  }
