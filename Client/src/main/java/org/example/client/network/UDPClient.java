...
public class UDPClient {
  ...
  private static final int SOCKET_TIMEOUT_MILLIS =
      5000; // Timeout for receiving a response from server
  ...
  public Response sendAndReceive(Command command) throws IOException, ClassNotFoundException {
    ...
    for (int retry = 0; retry < MAX_RETRIES; retry++) {
      try {
        ...
        System.out.println(
            "Sent command: " + command.getName() + " to " + serverAddress + ":" + serverPort);
    ...
    throw new IOException(
        "Failed to communicate with server after "
            + MAX_RETRIES
            + " retries. Server might be unavailable.");
  }
