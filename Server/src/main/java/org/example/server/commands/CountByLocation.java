...
public class CountByLocation implements ServerCommand {
  public static final String DESCRIPTION =
      "count_by_location {location}: count the number of elements whose location field value is equal to the given one";
  ...
  public Response execute(org.example.common.command.Command commandDto) {
    CountByLocationCommand countCommand =
        (CountByLocationCommand) commandDto; // Cast to specific DTO
    ...
    try {
      // SERVER-SIDE PARSING OF THE LOCATION STRING
      ...
      return new Response(
          "An unexpected error occurred while counting by location: " + e.getMessage(), false);
    }
  }
  ...
  private Location parseLocation(String arg) {
    ...
    if (parts.length < 3) {
      throw new IllegalArgumentException(
          "Location argument must be in format 'x y name'. Y can be empty if null.");
    }
    try {
      ...
      throw new IllegalArgumentException(
          "Invalid number format in location coordinates: " + e.getMessage());
    }
  }
